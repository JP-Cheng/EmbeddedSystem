  ...
  // sound-control part
  var trigThreshold = 0.2;
  ambient.on('ready', function () {
    ambient.setSoundTrigger(trigThreshold);
    ambient.on('sound-trigger', function trigHit() {
      relay.toggle(1);
    });
  });
  ...
  var server = http.createServer(function (request, response) {
    ...
    if (urlParts.pathname.match(infoRegex)) {
    ...
    if(index=='2'){
      ambient.getLightLevel( function(err, lightdata) {
        if (err) throw err;
                ...
                response.end(JSON.stringify(lightdata.toFixed(8)));
              }
      });
    }
    ...
    if (index=='3'){
      ambient.getSoundLevel( function(err, sounddata) {
        if (err) throw err;
          ...
          response.writeHead(200, {"Content-Type": "application/json"});
          response.end(JSON.stringify(sounddata.toFixed(8)));
        }
      });
    }
          /*
